...
func NewEthHandler(eth *Ethereum) func(*p2p.Peer) p2p.Protocol {
	return func(peer *p2p.Peer) p2p.Protocol {
		return NewEthProtocol(peer, eth)
	}
}

func NewEthProtocol(peer *p2p.Peer, eth *Ethereum) p2p.Protocol {
	return &EthProtocol{
		peer:     peer,
		ethereum: eth,
	}
